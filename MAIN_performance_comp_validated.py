...
case_name  = "Smart_Blade_Validated"
...
# VALIDATED VARIANT: use Jorge's genuine +/-180 deg multi-Reynolds tabulated
# polars (generated by _gen_jorge_polars.py into Inputs/jorge_polars/). These
# already span the full angle range so no Viterna overshoot is forced in deep
# stall -- this is one of the two fixes that take the closure to ~4%.
_JP = "Inputs/jorge_polars"
polar_files = {
    "cylinder": {
        10000:   f"{_JP}/cylinder_Re10000.csv",
        50000:   f"{_JP}/cylinder_Re50000.csv",
        100000:  f"{_JP}/cylinder_Re100000.csv",
        200000:  f"{_JP}/cylinder_Re200000.csv",
        300000:  f"{_JP}/cylinder_Re300000.csv",
        400000:  f"{_JP}/cylinder_Re400000.csv",
        1000000: f"{_JP}/cylinder_Re1000000.csv",
        2000000: f"{_JP}/cylinder_Re2000000.csv",
    },
    "S822": {
        50000:   f"{_JP}/S822_Re50000.csv",
        100000:  f"{_JP}/S822_Re100000.csv",
        200000:  f"{_JP}/S822_Re200000.csv",
        300000:  f"{_JP}/S822_Re300000.csv",
        400000:  f"{_JP}/S822_Re400000.csv",
        1000000: f"{_JP}/S822_Re1000000.csv",
        2000000: f"{_JP}/S822_Re2000000.csv",
    },
    "S823": {
        50000:   f"{_JP}/S823_Re50000.csv",
        100000:  f"{_JP}/S823_Re100000.csv",
        200000:  f"{_JP}/S823_Re200000.csv",
        300000:  f"{_JP}/S823_Re300000.csv",
        400000:  f"{_JP}/S823_Re400000.csv",
        1000000: f"{_JP}/S823_Re1000000.csv",
        2000000: f"{_JP}/S823_Re2000000.csv",
    },
}
...
# --- Cp surface sweep ---
v_surface        = 8.0
# VALIDATED VARIANT: the validation panel comes from the fixed-RPM loads sweep,
# not the Cp surface. Cap the surface to a fast, runaway-safe grid (lambda<=10);
# the full 40x40/lambda->14 surface is produced by the main MAIN_performance_comp.
lambda_values    = np.linspace(1.0, 10.0, 25)
pitch_values_deg = np.linspace(0.0, 30.0, 25)
cp_min_plot      = 0
...
v_rated           = 12.0
v_cut_out         = 25.0
rpm_max           = 193.0   # VALIDATED: true operating speed (was 250 = controller cap)
...
# VALIDATED VARIANT references are Jorge's own published curves:
#   "rigid"    = Jorge powerCurve_v2 (the closure reference; gives ~4% high-wind)
#   "deformed" = Jorge deformed-blade run (kept to show his own data scatter)
REFERENCE_DATA = {
    "rigid":    """\
2,-116.956016428325,-0.607808265760158,78.4041280142156,-5.847800821
3.28571428571429,50.6109281394391,0.0593183326335411,185.899003647892,2.530546407
4.57142857142857,555.287036278915,0.241654975238463,321.303746431256,27.76435181
5.85714285714286,1662.29410215457,0.343941384504639,491.955669814285,83.11470511
7.14285714285714,3239.73648824557,0.369596470777469,661.238843646379,161.9868244
8.42857142857143,4231.09325703293,0.293781675954923,738.048037307905,211.5546629
9.71428571428572,4675.07123691067,0.212026251847243,763.656565325816,233.7535618
11,4819.61716729085,0.150545753056727,774.125251188859,240.9808584
12.2857142857143,4918.02918335423,0.11026149815452,804.374944680366,245.9014592
13.5714285714286,5086.25306975759,0.084597063923015,857.058621449593,254.3126535
14.8571428571429,5155.13133443743,0.0653533504682004,913.393020614981,257.7565667
16.1428571428571,5375.66428784468,0.0531281813779152,989.037736168773,268.7832144
17.4285714285714,5661.97603186734,0.0444648340573116,1077.11446458252,283.0988016
18.7142857142857,5831.65785399836,0.0369918703380123,1166.56388647764,291.5828927
20,5976.19668934298,0.0310576732734157,1262.58631169977,298.8098345""",
...
blade = BladeGeometry(file_path=geometry_file, n_sections=n_sections,
                     ...
                     r_is_span_offset=False)  # VALIDATED: R=2.5 m to match Jorge
blade.load_csv()
blade.Malla()
